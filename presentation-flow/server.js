...
io.on('connection', (socket) => {
  ...
  socket.on('admin:presentation:stage', (data) => {
    if (!checkAdmin(data?.password)) return socket.emit('admin:error', { error: 'Unauthorized' })
    if (state.setPresentationStage(data.stage)) broadcast('state:full', state.getPublicState())
  })

  socket.on('admin:presentation:advance', (data) => {
    if (!checkAdmin(data?.password)) return socket.emit('admin:error', { error: 'Unauthorized' })
    if (state.advancePresentation(data.direction === -1 ? -1 : 1)) {
      broadcast('state:full', state.getPublicState())
    }
  })
